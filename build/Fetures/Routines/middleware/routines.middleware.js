...
const validateWeekdayMiddleware = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const routine = yield routine_models_1.default.findOne({ _id: classFind.rutin_id });
        if (!routine) {
            return res.status(404).send({ message: 'Routine not found' });
        }
        // Period not created validations
        const findEnd = yield priode_Models_1.default.findOne({ rutin_id: classFind.rutin_id, priode_number: start });
        const findStartPriod = yield priode_Models_1.default.findOne({ rutin_id: classFind.rutin_id, priode_number: end });
        if (!findEnd) {
        ...
        const startPriodeAlreadyBooked = yield weakday_Model_1.default.findOne({ routine_id: classFind.rutin_id, num, start });
        if (startPriodeAlreadyBooked) {
            return res.status(404).send({ message: 'Start period is already booked' });
        }
        const endPriodeAlreadyBooked = yield weakday_Model_1.default.findOne({ routine_id: classFind.rutin_id, num, end });
        ...
        const mid = [];
        const allStart = yield weakday_Model_1.default.find({ num });
        const allEnd = yield weakday_Model_1.default.find({ num }, { end: 1 });
